...
corners['corner_poses'].insert(3,deepcopy(corners['corner_poses'][3]))
...
front = np.array(list(range(n_vert)))
faces = [front,front+n_vert]

edge0 = np.array([0,1])
front_edges = [(edge0+i)%n_vert for i in range(6)]
print('front_edges',front_edges)

side_faces = [np.hstack([f,np.flip(f+n_vert)]) for f in front_edges]
